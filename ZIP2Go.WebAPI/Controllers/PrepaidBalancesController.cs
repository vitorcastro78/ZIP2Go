...
{
    /// <summary>
    /// Controller para gerenciamento de saldos pré-pagos
    /// </summary>
    [ApiController]
    [Route("api/[controller]")]
    public class PrepaidBalancesController : ControllerBaseApi
    {
        private readonly IPrepaidBalancesService _prepaidBalancesService;
        private readonly ILogger<PrepaidBalancesController> _logger;

        public PrepaidBalancesController(
            IPrepaidBalancesService prepaidBalancesService,
            IHttpContextAccessor httpContext,
            IEasyCachingProvider cache,
            ILogger<PrepaidBalancesController> logger) : base(httpContext, cache, logger)
        {
            _prepaidBalancesService = prepaidBalancesService ?? throw new ArgumentNullException(nameof(prepaidBalancesService));
            _logger = logger;
        }
        ...
        public async Task<IActionResult> CreatePrepaidBalance([FromBody] PrepaidBalanceCreateRequest request)
        {
            return await ExecuteWithErrorHandlingAsync(async () =>
            {
                ...
                var result = await _prepaidBalancesService.CreatePrepaidBalanceAsync(request);
                _logger.LogInformation("Saldo pré-pago criado com sucesso: {PrepaidBalanceId}", result.Id);
                return Created($"/api/prepaidbalances/{result.Id}", result);
            });
        }
        ...
        public async Task<IActionResult> GetPrepaidBalanceById([FromRoute] string prepaidBalanceId)
        {
            return await ExecuteWithErrorHandlingAsync(async () =>
            {
                ...
                var result = await _prepaidBalancesService.GetPrepaidBalanceByIdAsync(prepaidBalanceId);
                return Ok(result);
            });
        }
            ...
            [FromQuery][Range(1, 99)] int? pageSize = null)
        {
            return await ExecuteWithErrorHandlingAsync(async () =>
            {
                var result = await _prepaidBalancesService.GetPrepaidBalancesAsync(cursor, pageSize);
                return Ok(result);
            });
        }
            ...
            [FromBody] PrepaidBalanceUpdateRequest request)
        {
            return await ExecuteWithErrorHandlingAsync(async () =>
            {
                ...
                var result = await _prepaidBalancesService.UpdatePrepaidBalanceAsync(prepaidBalanceId, request);
